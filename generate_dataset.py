...
# --- Redesigned Pydantic Schemas for Robustness ---


class DialogueTurn(BaseModel):
    # This remains the same, but we will be more forceful in the prompt.
    role: str = Field(...,
...
class TherapyProfile(BaseModel):
    # A more flexible profile that aligns with the LLM's preferred output.
    values: List[str] = Field(...,
                              description="A list of the user's core values.")
    goals: List[str] = Field(...,
                             description="A list of the user's therapeutic goals.")
    # Adding a catch-all for other fields the model might generate
...
class EvolvedTherapyProfile(TherapyProfile):
    # The key field for our evaluation
...
class Session1Response(BaseModel):
    # Schema now matches the top-level keys the model is trying to generate.
...
# --- Simplified and Corrected Prompts ---
...
2.  `profile`: An 'Evolved Profile' object. It MUST contain the original "values" and "goals", plus a new key "evolution_notes" that clearly describes the user's profound shift.
...
def generate_case_study(case_id: str, seed_text: str) -> Optional[Dict[str, Any]]:
    """Generates a full multi-session case study using the new robust schemas."""
